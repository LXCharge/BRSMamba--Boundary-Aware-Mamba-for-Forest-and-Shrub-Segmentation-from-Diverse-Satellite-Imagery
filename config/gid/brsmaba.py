...
from geoseg.datasets.gid_dataset import *
...
#  define the network
num_heads = 2
windown_size = [16, 16, 8, 4]
# windown_size = 8
decode_channels = [16, 16, 32, 64]
dropout = 0.2
net = BRSMamba(
...
# loss = SegFormerLoss(ignore_index=ignore_index)
# define the dataloader

train_dataset = GIDDataset(
    data_root=f"data/{dataset_name}/train",
    mode="train",
    mosaic_ratio=0.25,
    transform=train_aug,
)

val_dataset = GIDDataset(data_root=f"data/{dataset_name}/val", transform=val_aug)
test_dataset = GIDDataset(data_root=f"data/{dataset_name}/val", transform=val_aug)
