...
def car_aug(image, mask):
    ...
    resize_crop_1 = albu.Compose(
        [
            albu.Resize(
                width=int(image.shape[0] * 1.25), height=int(image.shape[1] * 1.25)
            ),
    ...
    resize_crop_2 = albu.Compose(
        [
            albu.Resize(
                width=int(image.shape[0] * 1.5), height=int(image.shape[1] * 1.5)
            ),
            albu.RandomCrop(width=image.shape[0], height=image.shape[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    resize_crop_3 = albu.Compose(
        [
            albu.Resize(
                width=int(image.shape[0] * 1.75), height=int(image.shape[1] * 1.75)
            ),
            albu.RandomCrop(width=image.shape[0], height=image.shape[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    resize_crop_4 = albu.Compose(
        [
            albu.Resize(
                width=int(image.shape[0] * 2.0), height=int(image.shape[1] * 2.0)
            ),
            albu.RandomCrop(width=image.shape[0], height=image.shape[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                if (
                    ...
                    if (
                        cf[4] > 1.0 and mode == "train"
                    ):  # ignore car_aug, no improvement
