...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [255, 255, 255]  # 白色(BGR)
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [0, 0, 255]  # 红色(BGR)
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [0, 255, 0]  # 绿色(BGR)
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [255, 255, 0]  # 青色(BGR)
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [0, 255, 255]  # 黄色(BGR)
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [255, 0, 0]  # 蓝色(BGR)
    return mask_rgb
...
def get_args():
    ...
    # arg("-l", "--last", type=bool, help="whether link last", default=False)
...
def main():
    ...
    # if args.last:
    test_weights_name = config.test_weights_name + ".ckpt"
    if not os.path.exists(os.path.join(config.weights_path, test_weights_name)):
        test_weights_name = os.listdir(config.weights_path)[-1]
    # else:
    args.output_path.mkdir(exist_ok=True, parents=True)
    # test_weights_name = os.listdir(config.weights_path)[-1]
    print("test_weights_name", test_weights_name)
