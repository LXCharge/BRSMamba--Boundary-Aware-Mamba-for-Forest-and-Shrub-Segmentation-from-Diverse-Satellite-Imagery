...
def get_args():
    ...
    arg("-m", "--model_name", type=str, help="Name of the model/log.", required=True)
    ...
    arg("-hd", "--head", type=int, help="Number of heads.", required=False)
    arg("-ws", "--window_size", type=int, help="Window size.", required=False)
    arg("-in_dt_blk", "--in_dt_blk", type=int, help="Use in dt block.", required=False)
...
def main():
    ...
    config = arg2cfg(args)
    print(f"log_name: {config.log_name}")
    if config.in_dt_blk is not None:
        in_dt_blk = config.in_dt_blk
        if config.in_dt_blk == 1:
            in_dt_blk = [False, False, False, True]
        elif config.in_dt_blk == 2:
            in_dt_blk = [False, False, True, True]
        elif config.in_dt_blk == 3:
            in_dt_blk = [False, True, True, True]
        elif config.in_dt_blk == -1:
            in_dt_blk = [True, False, False, False]
        elif config.in_dt_blk == -2:
            in_dt_blk = [True, True, False, False]
        elif config.in_dt_blk == -3:
            in_dt_blk = [True, True, True, False]
    atmbConfig = {
        ...
        "num_heads": [2] * 4,
        "in_dt": True,
        "aux": config.use_aux_loss,
        "las_decode_channels": 8,
        "in_dt_blk": in_dt_blk,
        "las_decode_channels": 8,
    }
    if hasattr(config, "head") and config.head is not None:
        atmbConfig["num_heads"] = [config.head] * 4
    if hasattr(config, "window_size") and config.window_size is not None:
        atmbConfig["window_size"] = [config.window_size] * 4
    print(atmbConfig)
    config.net = BRSMamba(**atmbConfig)
