...
def get_args():
    ...
    arg("--val", help="whether eval validation set", default=True, action="store_true")
...
def main():
    ...
    print("config.test_weights_name", config.test_weights_name)
            ...
            for i in range(raw_predictions.shape[0]):
                ...
                if args.val:
                    if not os.path.exists(os.path.join(args.output_path, mask_type)):
                        os.makedirs(os.path.join(args.output_path, mask_type))
                    evaluator.add_batch(
