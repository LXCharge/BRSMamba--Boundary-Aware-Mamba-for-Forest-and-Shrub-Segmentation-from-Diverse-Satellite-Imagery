...
def rest_lite(pretrained=True, weight_path="pretrained_ckpt/rest_lite.pth", **kwargs):
    model = ResT(
        embed_dims=[64, 128, 256, 512],
        num_heads=[1, 2, 4, 8],
        mlp_ratios=[4, 4, 4, 4],
    ...
    if pretrained and weight_path is not None:
        old_dict = torch.load(weight_path, weights_only=True)
...
class DependencyPath(nn.Module):
    def __init__(self, weight_path="pretrained_ckpt/rest_lite.pth"):
...
class BANet(nn.Module):
    def __init__(self, num_classes=6, weight_path="pretrained_ckpt/rest_lite.pth"):
        super(
            BANet, self
        ).__init__()  # path of pretrained weight file of ResT-lite  or None, recommend use.
        self.name = "BANet"
