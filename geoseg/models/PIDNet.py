...
def get_seg_model(
    ...
    pth="./pretrained_ckpt/PIDNet_L_ImageNet.pth.tar",
    augment=False,
):
    ...
    if imgnet_pretrained:
        pretrained_state = torch.load(pth, map_location="cpu", weights_only=False)[
            "state_dict"
        ]
        ...
        pretrained_dict = torch.load(pth, map_location="cpu", weights_only=False)
...
if __name__ == "__main__":
    ...
    # FPS = 1000 / latency
    # print(FPS)
    model = get_seg_model().to("cuda")
    input = torch.randn(3, 3, 512, 512).to("cuda")
    output = model(input)
    for i in output:
        print(i.shape)
