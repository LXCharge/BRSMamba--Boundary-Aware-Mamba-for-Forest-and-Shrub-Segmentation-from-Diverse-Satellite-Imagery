...
def load_url(url, model_dir="./pretrained_ckpt", map_location=None):
...
def mobilenetv2(pretrained=False, **kwargs):
    model = MobileNetV2_ori(n_class=1000, **kwargs)
    if pretrained:
        model.load_state_dict(
            load_url(
...
class DeepLab(nn.Module):
    def __init__(
        self, num_classes, backbone="mobilenet", pretrained=True, downsample_factor=16
    ):
...
if __name__ == "__main__":
    model = DeepLab(num_classes=7, backbone="efficientnet_b3")
    # model.eval()
