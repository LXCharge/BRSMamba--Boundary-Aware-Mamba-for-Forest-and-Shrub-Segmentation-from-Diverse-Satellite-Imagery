...
try:
    from .backbone.SAM import ImageEncoderViT, MaskDecoder, PromptEncoder, Sam
    from .backbone.sammodel.transformer import TwoWayTransformer
except ImportError:
    from backbone.SAM import ImageEncoderViT, MaskDecoder, PromptEncoder, Sam
    from backbone.sammodel.transformer import TwoWayTransformer
...
def draw_features(feature, savename=""):
    ...
    visualize = (
        ((visualize - np.min(visualize)) / (np.max(visualize) - np.min(visualize)))
        * 255
    ).astype(np.uint8)
...
def parse_args():
    ...
    parser.add_argument(
        "-mod", type=str, default="sam_lora", help="mod type:seg,cls,val_ad"
    )
...
class MFNet(nn.Module):
    def __init__(self, decode_channels=64, dropout=0.1, window_size=8, num_classes=6):
        ...
        self.sam = build_sam_vit_b(
            args,
            checkpoint="/home/hzj/projects/experiment/BRSMamba/Code/ERSMNet/pretrained_ckpt/sam_vit_b_01ec64.pth",
        )
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        new_state_dict = {
            ...
            if k in sam.state_dict() and sam.state_dict()[k].shape == v.shape
        }
...
if __name__ == "__main__":
    net = MFNet(num_classes=6).to("cuda")
