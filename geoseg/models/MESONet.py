...
def one_hot(label, n_classes, requires_grad=False, ignore_index=255):
    ...
    one_hot_label[:, :, :, 0] = torch.where(
        ((label >= 0) & (label == ignore_index)),
        zeros,
        one_hot_label[:, :, :, 0].type(torch.LongTensor).to(device),
    )
...
class EPFO_Head(nn.Module):
    ...
    def forward(
        ...
        point_indices, points = self.get_points_test(
            coarse_label, boundry, calculate_uncertainty, cfg=test_cfg
        )
        ...
        point_logits = self.ori_forward(fine_grained_point_feats, coarse_point_feats)
        ...
        return self.predict_by_feat(
            MSCA_out, boundry, points, coarse_label, batch_img_metas, **kwargs
        )

    def ori_forward(self, fine_grained_point_feats, coarse_point_feats):
    ...
    def get_points_train(self, seg_logits, boundry, uncertainty_func, cfg):
        boundry_coords = [
            (torch.nonzero(boundry[i] > 0, as_tuple=False) / (self.channels - 1)).shape[
                0
            ]
            for i in range(boundry.shape[0])
        ]
        ...
        list_zeros = [
            (torch.sum(uncertainty_map_zeros[i] != 0))
            for i in range(uncertainty_map_zeros.shape[0])
        ]

        for i in range(len(list_zeros)):
            if list_zeros[i] >= num_points:
                uncertainty_map0 = uncertainty_map[i : i + 1, :, :, :].clone()
                uncertainty_map0[uncertainty_map_zeros[i : i + 1, :, :, :] == 0] = (
                    torch.min(uncertainty_map0) - 1.0
                )
                uncertainty_map0 = uncertainty_map0.view(1, height * width)
                point_indices = uncertainty_map0.topk(num_points, dim=1)[1]

                point_coords = torch.zeros(
                    1, num_points, 2, dtype=torch.float, device=seg_logits.device
                )
                ...
                uncertainty_map1[uncertainty_map_zeros[i : i + 1, :, :, :] == 0] = (
                    torch.min(uncertainty_map1) - 1.0
                )
                ...
                point_coords[:, : list_zeros[i], 0] = (
                    point_indices0 % width
                ).float() * w_step
                point_coords[:, : list_zeros[i], 1] = (
                    point_indices0 // width
                ).float() * h_step

                uncertainty_map1 = uncertainty_map[i : i + 1, :, :, :].clone()
                uncertainty_map_zeros = uncertainty_map[i : i + 1, :, :, :] * (
                    1 - boundry
                ).unsqueeze(1)
                uncertainty_map1[uncertainty_map_zeros[i : i + 1, :, :, :] == 0] = (
                    torch.min(uncertainty_map1) - 1.0
                ...
                point_indices1 = uncertainty_map1.topk(
                    num_points - list_zeros[i], dim=1
                )[1]

                point_coords[:, list_zeros[i] :, 0] = (
                    point_indices1 % width
                ).float() * w_step
                point_coords[:, list_zeros[i] :, 1] = (
                    point_indices1 // width
                ).float() * h_step
...
class MESONet(nn.Module):
    ...
    def __init__(
        ...
        maca_in_channels = sum(self.backbone.feature_info.channels()[1:])
        self.msca = MSCA_Head(
            in_channels=maca_in_channels,
        )

    def forward(self, x):
        features = self.backbone(x)
        msca_out = self.msca(features)
        return msca_out
