...
try:
    ...
    from backbone.CNNVIT import CNNTblock
    from backbone.Mobilevit import (
        MobileViTBlock,
        MobileViTBlocktinytem2,
        MobileViTBlocktinytem,
        MobileViTBlocktemSCPE,
        MobileViTBlockQVtemSCPE,
        MobileViTBlocktemtemSCPE,
        MobileViTBlocktemnew,
        MobileViTBlocktemtem,
    )
    from backbone.Mobilevit import (
        MobileViTBlock,
        MobileViTBlocktem,
    ...
    from .backbone.Mobilevit import MobileViTBlock, MobileViTBlocktem, MobileViTBlocktem
...
class CMUNeXtBlock_MK(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
        ch_in,
        ch_out,
...
class CMUNeXtBlock_MK_resiual1(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
...
class CMUNeXtBlock_MK_resiual2(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
...
class CMUNeXtBlock_MK1_resiual(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
...
class CMUNeXtBlock_MK2_resiual(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
...
class CMUNeXtBlock_MK2_resiual2(
    nn.Module
):  # 根据SCSA灵感，直接在一个block中为不同的channel设置不同的卷积核，同时引入通道注意力
    def __init__(
        self,
...
class CMUNeXtBlock_MMK(
    ...
    def __init__(
        ...
        self.BatchNorm2d = nn.ModuleList(
            [nn.BatchNorm2d(self.group_chans) for i in range(4)]
        )
        ...
        x_attn = self.norm_act(
            torch.cat(
                (
                    ...
                    self.BatchNorm2d[1](
                        self.local_dwc[1](g_x_s) + self.global_dwc_s[0](g_x_s)
                    ),
                    self.BatchNorm2d[2](
                        self.local_dwc[2](g_x_m)
                        + self.global_dwc_s[1](g_x_m)
                        + self.global_dwc_m[0](g_x_m)
                    ),
                    self.BatchNorm2d[3](
                        self.local_dwc[3](g_x_l)
                        + self.global_dwc_s[2](g_x_l)
                        + self.global_dwc_m[1](g_x_l)
                        + self.global_dwc_l(g_x_l)
                    ),
                ),
                dim=1,
...
class fusion_conv(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                ch_in, ch_in, kernel_size=3, stride=1, padding=1, groups=2, bias=True
            ),
            nn.GELU(),
            nn.BatchNorm2d(ch_in),
...
class CMUNeXt(nn.Module):
    def __init__(
        ...
        self.Conv_1x1 = nn.Conv2d(
            dims[0], num_classes, kernel_size=1, stride=1, padding=0
        )
...
class CMUNeXt_MK(nn.Module):
    def __init__(
        ...
        self.encoder1 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[0], ch_out=dims[0]) for i in range(depths[0])],
            conv_1x1_bn(dims[0], dims[0])
        )
        self.encoder2 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[0], ch_out=dims[1]) for i in range(depths[1])],
            conv_1x1_bn(dims[0], dims[1])
        )
        self.encoder3 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[1], ch_out=dims[2]) for i in range(depths[2])],
            conv_1x1_bn(dims[1], dims[2])
        )
        self.encoder4 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[2], ch_out=dims[3]) for i in range(depths[3])],
            conv_1x1_bn(dims[2], dims[3])
        )
        self.encoder5 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[3], ch_out=dims[4]) for i in range(depths[4])],
            conv_1x1_bn(dims[3], dims[4])
        )
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = fusion_conv(ch_in=dims[3] * 2, ch_out=dims[3])
...
class CMUNeXt_MK_VIT_1(nn.Module):
    def __init__(
        ...
        self.encoder5 = nn.Sequential(
            *[CMUNeXtBlock_MK(ch_in=dims[3], ch_out=dims[4]) for i in range(depths[4])],
            ...
            MobileViTBlock(
                ...
                kernel_size=3,
                patch_size=(2, 2),
                mlp_dim=int(dims[4] * 2),
            )
        )
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = nn.Sequential(
            fusion_conv(ch_in=dims[3] * 2, ch_out=dims[3]),
            MobileViTBlock(
                dims[3],
                1,
                dims[3],
        ...
        self.Up_conv4 = nn.Sequential(
            fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2]),
            MobileViTBlock(
                dims[2],
                1,
                dims[2],
...
class CMUNeXt_MK_resiual(nn.Module):
    def __init__(
        ...
        self.encoder1 = nn.Sequential(
            *[
                CMUNeXtBlock_MK_resiual(ch_in=dims[0], ch_out=dims[0])
        ...
        self.encoder2 = nn.Sequential(
            *[
                CMUNeXtBlock_MK_resiual(ch_in=dims[0], ch_out=dims[1])
        ...
        self.encoder3 = nn.Sequential(
            *[
                CMUNeXtBlock_MK_resiual(ch_in=dims[1], ch_out=dims[2])
        ...
        self.encoder4 = nn.Sequential(
            *[
                CMUNeXtBlock_MK_resiual(ch_in=dims[2], ch_out=dims[3])
        ...
        self.encoder5 = nn.Sequential(
            *[
                CMUNeXtBlock_MK_resiual(ch_in=dims[3], ch_out=dims[4])
...
class CMUNeXt_MK_resiual_model(nn.Module):
    def __init__(
        ...
        self.encoder1 = nn.Sequential(
            *[
                CMUNeXtBlockmodel(ch_in=dims[0], ch_out=dims[0])
                for i in range(depths[0])
            ],
            conv_1x1_bn(dims[0], dims[0])
        )
        self.encoder2 = nn.Sequential(
            *[
                CMUNeXtBlockmodel(ch_in=dims[0], ch_out=dims[1])
                for i in range(depths[1])
            ],
            conv_1x1_bn(dims[0], dims[1])
        )
        self.encoder3 = nn.Sequential(
            *[
                CMUNeXtBlockmodel(ch_in=dims[1], ch_out=dims[2])
                for i in range(depths[2])
            ],
            conv_1x1_bn(dims[1], dims[2])
        )
        self.encoder4 = nn.Sequential(
            *[
                CMUNeXtBlockmodel(ch_in=dims[2], ch_out=dims[3])
                for i in range(depths[3])
            ],
            conv_1x1_bn(dims[2], dims[3])
        )
        self.encoder5 = nn.Sequential(
            *[
                CMUNeXtBlockmodel(ch_in=dims[3], ch_out=dims[4])
                for i in range(depths[4])
            ],
            conv_1x1_bn(dims[3], dims[4])
        )
...
class DU_MK_resiual_model(nn.Module):
    def __init__(
        ...
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = nn.Sequential(
            conv_1x1_bn(dims[3] * 2, dims[3] * 2),
            CMUNeXtBlockmodel(ch_in=dims[3] * 2, ch_out=dims[3] * 2),
            conv_1x1_bn(dims[3] * 2, dims[3]),
        )
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = nn.Sequential(
            conv_1x1_bn(dims[2] * 2, dims[2] * 2),
            CMUNeXtBlockmodel(ch_in=dims[2] * 2, ch_out=dims[2] * 2),
            conv_1x1_bn(dims[2] * 2, dims[2]),
        )
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
        self.Up_conv3 = nn.Sequential(
            conv_1x1_bn(dims[1] * 2, dims[1] * 2),
            CMUNeXtBlockmodel(ch_in=dims[1] * 2, ch_out=dims[1] * 2),
            conv_1x1_bn(dims[1] * 2, dims[1]),
        )
        self.Up2 = up_conv(ch_in=dims[1], ch_out=dims[0])
        self.Up_conv2 = nn.Sequential(
            conv_1x1_bn(dims[0] * 2, dims[0] * 2),
            CMUNeXtBlockmodel(ch_in=dims[0] * 2, ch_out=dims[0] * 2),
            conv_1x1_bn(dims[0] * 2, dims[0]),
        )
        self.Conv_1x1 = nn.Conv2d(
            dims[0], num_classes, kernel_size=1, stride=1, padding=0
        )
...
class CMUNeXt_MK_resiual_model_1(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktem(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvit1(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktinytem(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvit3(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktinytem2(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvittemtem4(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktemtem(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvittemnew4(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktemnew(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvittemtem4_SCPE(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktemtemSCPE(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class MKtinyvit2SCPE(nn.Module):
    def __init__(
        ...
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlocktemSCPE(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
    ...
    def __init__(
        ...
        self.encoder5 = nn.Sequential(
            MobileViTBlockQVtemSCPE(
                ...
                mlp_dim=int(dims[4] * 2),
                pos_len=16 // 2 * 16 // 2,
                hw=[int(16 // 2), int(16 // 2)],
                attention=attention,
            )
        )
        # Decoder
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = MobileViTBlockQVtemSCPE(
            dims[3] * 2,
            1,
            dims[3],
            kernel_size=3,
...
class CMUNeXt_MMK(nn.Module):
    def __init__(
        ...
        self.encoder1 = nn.Sequential(
            *[
                CMUNeXtBlock_MMK(ch_in=dims[0], ch_out=dims[0])
                for i in range(depths[0])
            ],
            conv_1x1_bn(dims[0], dims[0])
        )
        self.encoder2 = nn.Sequential(
            *[
                CMUNeXtBlock_MMK(ch_in=dims[0], ch_out=dims[1])
                for i in range(depths[1])
            ],
            conv_1x1_bn(dims[0], dims[1])
        )
        self.encoder3 = nn.Sequential(
            *[
                CMUNeXtBlock_MMK(ch_in=dims[1], ch_out=dims[2])
                for i in range(depths[2])
            ],
            conv_1x1_bn(dims[1], dims[2])
        )
        self.encoder4 = nn.Sequential(
            *[
                CMUNeXtBlock_MMK(ch_in=dims[2], ch_out=dims[3])
                for i in range(depths[3])
            ],
            conv_1x1_bn(dims[2], dims[3])
        )
        self.encoder5 = nn.Sequential(
            *[
                CMUNeXtBlock_MMK(ch_in=dims[3], ch_out=dims[4])
                for i in range(depths[4])
            ],
            conv_1x1_bn(dims[3], dims[4])
        )
...
class CMUNeXt_MKK(nn.Module):
    def __init__(
        ...
        self.encoder1 = nn.Sequential(
            *[
                CMUNeXtBlock_MKUP(ch_in=dims[0], ch_out=dims[0])
                for i in range(depths[0])
            ],
            conv_1x1_bn(dims[0], dims[0])
        )
        ...
        self.encoder3 = nn.Sequential(
            *[
                CMUNeXtBlock_MKUP(ch_in=dims[1], ch_out=dims[1])
                for i in range(depths[2])
            ],
            conv_1x1_bn(dims[1], dims[2])
        )
        self.encoder4 = nn.Sequential(
            *[
                CMUNeXtBlock_MKUP(ch_in=dims[2], ch_out=dims[2])
                for i in range(depths[3])
            ],
            conv_1x1_bn(dims[2], dims[3])
        )
        self.encoder5 = nn.Sequential(
            *[
                CMUNeXtBlock_MKUP(ch_in=dims[3], ch_out=dims[3])
                for i in range(depths[4])
            ],
            conv_1x1_bn(dims[3], dims[4])
        )
...
class CMUNeXt_CNNVIT_v1(nn.Module):
    def __init__(
        ...
        self.stem = conv_block(ch_in=input_channel, ch_out=dims[0])
        self.encoder1 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[0], ch_out=dims[0], depth=depths[0], k=kernels[0])
        )
        self.encoder2 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[0], ch_out=dims[1], depth=depths[1], k=kernels[1]),
            CNNTblock(
                in_channels=dims[1], out_channels=dims[1], kernel_sizes=[5, 9, 13]
            ),
        )
        self.encoder3 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[1], ch_out=dims[2], depth=depths[2], k=kernels[2]),
            CNNTblock(
                in_channels=dims[2], out_channels=dims[2], kernel_sizes=[5, 9, 13]
            ),
        )
        self.encoder4 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[2], ch_out=dims[3], depth=depths[3], k=kernels[3]),
            CNNTblock(
                in_channels=dims[3], out_channels=dims[3], kernel_sizes=[5, 9, 13]
            ),
        )
        self.encoder5 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[3], ch_out=dims[4], depth=depths[4], k=kernels[4]),
            CNNTblock(
                in_channels=dims[4], out_channels=dims[4], kernel_sizes=[5, 9, 13]
            ),
        )
...
class CMUNeXt_CNNVIT_v2(nn.Module):
    def __init__(
        ...
        self.encoder4 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[2], ch_out=dims[3], depth=depths[3], k=kernels[3]),
            CNNTblock(in_channels=dims[3], out_channels=dims[3]),
        )
        self.encoder5 = nn.Sequential(
            CMUNeXtBlock(ch_in=dims[3], ch_out=dims[4], depth=depths[4], k=kernels[4]),
            CNNTblock(in_channels=dims[4], out_channels=dims[4]),
        )
...
class CMUNeXt_CBAM_v1(nn.Module):
    def __init__(
        ...
        self.Up5 = up_conv(ch_in=dims[4], ch_out=dims[3])
        self.Up_conv5 = nn.Sequential(
            fusion_conv(ch_in=dims[3] * 2, ch_out=dims[3]), CBAM(dims[3])
        )
        self.Up4 = up_conv(ch_in=dims[3], ch_out=dims[2])
        self.Up_conv4 = fusion_conv(ch_in=dims[2] * 2, ch_out=dims[2])
        self.Up3 = up_conv(ch_in=dims[2], ch_out=dims[1])
...
class CMUNeXt_for_skip(nn.Module):
    def __init__(
        ...
        self.encoder2_for_skip = CMUNeXtBlock(
            ch_in=dims[0], ch_out=dims[0], depth=depths[1], k=kernels[1]
        )
...
class CMUNeXt_for_skip_3(nn.Module):
    def __init__(
        ...
        self.encoder3_for_skip = CMUNeXtBlock(
            ch_in=dims[1], ch_out=dims[1], depth=depths[2], k=kernels[2]
        )
        self.encoder4 = CMUNeXtBlock(
            ch_in=dims[2], ch_out=dims[3], depth=depths[3], k=kernels[3]
        )
        self.encoder4_for_skip = CMUNeXtBlock(
            ch_in=dims[2], ch_out=dims[2], depth=depths[3], k=kernels[3]
        )
        self.encoder5 = CMUNeXtBlock(
            ch_in=dims[3], ch_out=dims[4], depth=depths[4], k=kernels[4]
        )
...
class CMUNeXt_ver3(nn.Module):
    def __init__(
        ...
        self.encoder6 = CMUNeXtBlock(
            ch_in=dims[4], ch_out=dims[5], depth=depths[5], k=kernels[5]
        )
