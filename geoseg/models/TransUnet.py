...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        x_path_size = x.shape[2:]
        x = x.flatten(2)
        x = x.transpose(-1, -2)  # (B, n_patches, hidden)
        x_pos = F.interpolate(
            self.position_embeddings.transpose(-1, -2).reshape(
                1,
        ...
        embeddings = x + x_pos.flatten(2).transpose(-1, -2)
...
class TransUnet(nn.Module):
    def __init__(
        self, config, img_size=512, num_classes=21843, zero_head=False, vis=False
    ):
...
def get_b16_config():
    ...
    config.pretrained_path = "./pretrained_ckpt/imagenet21k_ViT-B_16.npz"
...
if __name__ == "__main__":
    ...
    model = TransUnet(config, img_size=512, num_classes=6).cuda()
    input = torch.randn(12, 3, 512, 512).cuda()
    output = model(input)
    print(output.shape)
