            ...
            OrderedDict(
                [
                    (
                        "conv",
                        StdConv2d(
                            1, width, kernel_size=7, stride=2, bias=False, padding=3
                        ),
                    ),
                    ("gn", nn.GroupNorm(32, width, eps=1e-6)),
                    ("relu", nn.ReLU(inplace=True)),
        ...
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                ...
                for (
                    bname,
                    block,
                ) in self.transformer.embeddings.hybrid_model.bodyd.named_children():
                    for uname, unit in block.named_children():
                        unit.load_from(res_weight, n_block=bname, n_unit=uname)
...
if __name__ == "__main__":
    ...
    model = FTransUnet(config, img_size=512, num_classes=6).cuda()
