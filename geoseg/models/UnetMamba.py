try:
    from .backbone.VMamba import (
        VSSM,
        LayerNorm2d,
        VSSBlock,
        Permute,
    )
    from .backbone.ResT import ResT
except:
    from backbone.VMamba import (
        VSSM,
        LayerNorm2d,
...
def rest_lite(pretrained=True, weight_path="pretrain_weights/rest_lite.pth", **kwargs):
    ...
    if pretrained and weight_path is not None:
        old_dict = torch.load(weight_path, weights_only=False)
...
class UNetMamba(nn.Module):
    def __init__(
        ...
        backbone_path="pretrained_ckpt/rest_lite.pth",
        embed_dim=64,
        num_classes=6,
        **kwargs,
    ):
