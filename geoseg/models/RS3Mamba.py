...
from .backbone.SwinUMamba import VSSMEncoder
...
class ChannelAtt(nn.Module):
    def __init__(
        self, gate_channels, reduction_ratio=2, pool_types=["avg", "max", "soft"]
    ):
    ...
    def forward(self, x):
        ...
        self.pool = SoftPool2d(
            kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3))
        )
...
class RS3Mamba(nn.Module):
    def __init__(
        ...
        encoder_channels = backbone.feature_info.channels()
    ...
    def forward(self, x):
        ...
        vss_outs = self.vssm_encoder(
            ssmx
        )  # 48*128*128, 96*64*64, 192*32*32, 384*16*16, 768*8*8
