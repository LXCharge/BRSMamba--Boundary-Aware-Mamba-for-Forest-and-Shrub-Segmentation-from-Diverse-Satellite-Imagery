...
class ChannelAtt(nn.Module):
    ...
    def forward(self, x, fre=False):
        ...
        if fre:
            ...
            atten = torch.mean(h_tv, dim=(2, 3), keepdim=True) + torch.mean(
                w_tv, dim=(2, 3), keepdim=True
            )
        else:
...
class RSAM(nn.Module):
    ...
    def forward(self, sp_feat, co_feat):
        ...
        re_s_att = torch.sigmoid(
            s_att + self.g1 * sp_mlp_out.unsqueeze(-1).unsqueeze(-1)
        )
        re_c_att = torch.sigmoid(
            c_att + self.g2 * co_mlp_out.unsqueeze(-1).unsqueeze(-1)
        )
        c_feat = torch.mul(c_feat, re_c_att)
        s_feat = torch.mul(s_feat, re_s_att)
        c_feat = F.interpolate(
            c_feat, s_feat.size()[2:], mode="bilinear", align_corners=False
        )
...
class LowPassModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = [
            F.interpolate(input=self.stages[i](feats[i]), size=(h, w), mode="bilinear")
            for i in range(4)
        ]
...
class FilterModule(nn.Module):
    def __init__(self, Ch, h=8, window={3: 2, 5: 3, 7: 3}):
    ...
    def forward(self, q, v, size):
        ...
        HP_list = [conv(x) for conv, x in zip(self.conv_list, v_img_list)]
...
class IFM(nn.Module):
    ...
    def forward(self, x):
        ...
        df_q, df_k, df_v = rearrange(
            df_qkv, "b (qkv c h) n -> qkv b h n c", h=self.num_heads, qkv=3
        )
        # end
        ...
        df = rearrange(
            df,
            "b h (hh ws1 ww ws2) d -> (b hh ww) h (ws1 ws2) d",
            h=self.num_heads,
            d=C // self.num_heads,
            hh=Hp // self.ws,
            ww=Wp // self.ws,
...
class MIFNet(nn.Module):
    def __init__(
        ...
        self.res1_dim = Conv(
            encoder_channels[0], decoder_input_channels[0], kernel_size=1
        )
        self.res2_dim = Conv(
            encoder_channels[1], decoder_input_channels[1], kernel_size=1
        )
        self.res3_dim = Conv(
            encoder_channels[2], decoder_input_channels[2], kernel_size=1
        )
        self.res4_dim = Conv(
            encoder_channels[3], decoder_input_channels[3], kernel_size=1
        )

        self.decoder = Decoder(
            decoder_input_channels, decode_channels, dropout, window_size, num_classes
        )
...
def main():
    model = MIFNet(backbone_name="efficientnet_b3").cuda()
    input = torch.randn(8, 3, 512, 512).cuda()
    output = model(input)
    print(output.shape)
