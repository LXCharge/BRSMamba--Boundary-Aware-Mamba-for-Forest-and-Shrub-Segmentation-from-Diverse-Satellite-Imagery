...
class E_FFN(nn.Module):
    def __init__(
        ...
        self.fc1 = ConvBNReLU(
            in_channels=in_features, out_channels=hidden_features, kernel_size=1
        )
...
class Mutilscal_MHSA(nn.Module):
    def __init__(
        ...
        self.MSC = MutilScal(
            dim=dim, fc_ratio=fc_ratio, dilation=dilation, pool_ratio=pool_ratio
        )
...
class MAF(nn.Module):
    def __init__(self, dim, fc_ratio, dilation=[3, 5, 7], dropout=0.0, num_classes=6):
...
class CMTFNet(nn.Module):
    ...
    def __init__(
        ...
        encode_channels = self.backbone.feature_info.channels()
        self.decoder = Decoder(
            encode_channels, decode_channels, dropout=dropout, num_classes=num_classes
        )
