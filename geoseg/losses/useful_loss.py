...
class AtMbLoss(nn.Module):

    def __init__(self, ignore_index=255, edge_factor=1.0, alpha=0.4):
        super().__init__()
        self.alpha = alpha
        self.main_loss = JointLoss(
            SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.main_loss(logit_main, labels) + self.alpha * self.aux_loss(
                logit_aux, labels
            )
        else:
            loss = self.main_loss(logits, labels)
...
if __name__ == "__main__":
    targets = torch.randint(low=0, high=2, size=(2, 16, 16)).cuda()
    logit_main = torch.randn((2, 2, 16, 16)).cuda()
    logit_aux = torch.randn((2, 2, 16, 16)).cuda()
    # print(targets)
    model = AtMbLoss().cuda()
    model.train()
    loss = model((logit_main, logit_aux), targets)
